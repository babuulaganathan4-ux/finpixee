...
const VendorPortalPage: React.FC<VendorPortalProps> = ({ onLogout }) => {
    ...
    const handleRemoveGstRecord = (id: string) => {
        if (gstRecords.length > 1) {
            setGstRecords(gstRecords.filter((record: GSTRecord) => record.id !== id));
        }
    };

    const handleGstChange = (id: string, field: keyof GSTRecord, value: any) => {
        setGstRecords(gstRecords.map((record: GSTRecord) => {
            if (record.id === id) {
    ...
    const handleFetchGstDetails = async (id: string) => {
        ...
        // Simulate API call
        setTimeout(() => {
            setGstRecords(gstRecords.map((r: GSTRecord) => {
                if (r.id === id) {
                    return {
                        ...r,
    ...
    const toggleGstExpand = (id: string) => {
        setGstRecords(gstRecords.map((r: GSTRecord) => r.id === id ? { ...r, isExpanded: !r.isExpanded } : r));
    };
    ...
    const togglePobExpand = (recordId: string, pobId: string) => {
        setGstRecords(gstRecords.map((r: GSTRecord) => {
            if (r.id === recordId) {
                return {
                    ...r,
                    placesOfBusiness: r.placesOfBusiness.map((p: PlaceOfBusiness) =>
    ...
    const handleRemovePob = (recordId: string, pobId: string) => {
        setGstRecords(gstRecords.map((r: GSTRecord) => {
            if (r.id === recordId) {
                return {
                    ...r,
                    placesOfBusiness: r.placesOfBusiness.filter((p: PlaceOfBusiness) => p.id !== pobId)
                };
            }
            return r;
        }));
